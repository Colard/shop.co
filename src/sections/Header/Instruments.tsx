...
let Instruments: React.FC<InstrumentsProps> = ({ darkModeToggler }) => {
  const buttonClassName = tw`hover:before:bg-primary/30 relative size-6 transform transition-all duration-300 before:absolute before:inset-0 before:rounded-full before:transition-all before:duration-300 hover:scale-125 hover:before:scale-130`;
  ...
  return (
    <div className="flex items-center gap-3 md:contents md:gap-4">
      <div className="md:bg-secondary mx-10 contents h-12 flex-1 items-center rounded-full px-4 md:flex">
        <SVGButton
          svg={<SearchIcon />}
          className={`${buttonClassName} md:opacity-40 md:transition-none md:duration-0 md:before:hidden md:hover:scale-100`}
        ></SVGButton>
        <input
